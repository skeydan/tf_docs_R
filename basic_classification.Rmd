...
```
Note - googled for ways to produce something like the original plot but didn't really find anything.
For example, this:
https://tensorflow.rstudio.com/tfestimators/articles/examples/mnist.html
does the plotting after dividing by 255, while it seems the narrative here is showing the original scale before division...
Would this make for an alternative?
```

```{r}
library(ggplot2)
first_image_pixels <- data.frame(pixel = as.vector(train_images[1, , ]))
ggplot(first_image_pixels, aes(x = pixel)) + geom_density()
```
