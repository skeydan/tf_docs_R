...
dataset <- tensor_slices_dataset(tuple(data, labels)) %>% 
           dataset_batch(32, drop_remainder = TRUE) %>%
           dataset_repeat()
...
API](https://tensorflow.rstudio.com/keras/articles/functional_api.html)
...
x <- layer_dense(units = 64, activation = 'relu')(inputs)
x <- layer_dense(units = 64, activation = 'relu')(x)
predictions <- layer_dense(units = 10, activation = 'softmax')(x)
...
While the above code mimics the Python syntax and emphasizes the difference to the `sequential` model, in R you can equivalently write
...
This is because each of the `layer_dense` functions, called on a tensor, returns a tensor itself.
...
To use eager execution from R, you need to tell Keras to use the TensorFlow implementation of Keras (as opposed to native Keras):
...
The [Estimators](https://tensorflow.rstudio.com/tfestimators/) API is used for training models for distributed environments. This targets industry use cases such as
distributed training on large datasets that can export a model for
production.

To use `tfestimators`, you need to install separately:
...
A `model` can be trained with the `tfestimators` API by
converting the model to an `estimator` object with
`keras_model_to_estimator`. 
Note: As of today this only works with the TensorFlow implementation of Keras.
...
Note: You can enable eager execution for debugging [estimator
input functions](https://tensorflow.rstudio.com/tfestimators/articles/estimator_basics.html) and
inspecting data.

### Multiple GPUs

You can run Keras models in parallel on multiple GPUs either using [Keras `multi_gpu_model()`](https://keras.rstudio.com/articles/faq.html#how-can-i-run-a-keras-model-on-multiple-gpus) or using `estimators`with `tf$contrib$distribute$DistributionStrategy`. 
Please see the above link for Keras `multi_gpu_model`. 
The `tfestimators`API provides distributed training on multiple GPUs with almost no changes to existing code.
...
Keras model to an estimator with `keras_model_to_estimator`, then train the estimator.

The following example distributes a model across multiple
GPUs on a single machine.

First, define a simple model.
Note: As of today this only works with the TensorFlow implementation of Keras, so line 2 is required.

```{r}
library(keras)
use_implementation("tensorflow")
model <- keras_model_sequential()
model %>% layer_dense(units = 16, activation = 'relu', input_shape = 10)
...
Define an *input pipeline*. The `input_fn` returns a `dataset`
...
library(tfdatasets)
input_fn <- function() {
  x <- matrix(rnorm(1024 * 10), nrow = 1024, ncol = 10)
  y <- matrix(sample(0:1, 1024, replace = TRUE), nrow = 1024)
  dataset <- tensor_slices_dataset(tuple(x, y)) %>%
    dataset_repeat(10) %>%
    dataset_batch(32)
  dataset
}
...
```{r}
strategy <- tf$contrib$distribute$MirroredStrategy(num_gpus = 4L)
config <- tf$estimator$RunConfig(train_distribute = strategy)
```

Convert the Keras model to an `estimator` instance:
